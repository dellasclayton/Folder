...
export function initChat() {
  ...
  bindCharacterCacheHandlers()
  syncActiveCharacters()

  console.log('[Chat] Initialized')
}

/**
 * Sync active characters from cache
 */
async function syncActiveCharacters() {
  try {
    ...
    console.log(`[Chat] Synced ${activeCount} active characters`)
  } catch (error) {
    console.error('[Chat] Failed to load characters:', error)
  }
}

function bindCharacterCacheHandlers() {
  if (cacheHandlersBound) return

  const handleCacheChange = () => {
    syncActiveCharacters()
  }

  characterCache.on('character:created', handleCacheChange)
  characterCache.on('character:updated', handleCacheChange)
  characterCache.on('character:deleted', handleCacheChange)
  characterCache.on('cache:refreshed', handleCacheChange)
  characterCache.on('cache:initialized', handleCacheChange)

  cacheHandlersBound = true
}
